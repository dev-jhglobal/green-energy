...
(function() {
        ...
        cont.find('a').each(function() {
            ...
            if (href && href != '#' && !trx_addons_is_local_link(href) && !link.data('trx-addons-extra-args-added')) {
                ...
                for (var i = 0; i < args.length; i++) {
                    ...
                    if (page_valid && (!args[i].mask || href.indexOf(args[i].mask) >= 0) && (!args[i].link || href.indexOf(args[i].link) < 0)) {
                        var add = false;
    ...
    window.trx_addons_parse_ajax_response = function(response, error_msg = '') {
        ...
        if (response == '' || response == 0) {
            rez = {
                error: error_msg || TRX_ADDONS_STORAGE['msg_ajax_error']
            };
    ...
    window.trx_addons_get_embed_from_url = function(url, autoplay, mute, loop) {
        ...
        if (url.indexOf('.mp4') > 0 || trx_addons_is_local_link(url)) {
            ...
            url = url.replace('/watch?v=', '/embed/').replace('/youtu.be/', '/www.youtube.com/embed/').replace(new RegExp(/\/vimeo\.com\/(\d+)(?:\/([a-z0-9]+))?/, 'i'), '/player.vimeo.com/video/$1?h=$2').replace('/dai.ly/', '/dailymotion.com/embed/video/').replace('/dailymotion.com/video/', '/dailymotion.com/embed/video/');
            if (autoplay) {
        ...
        if (str.indexOf('[') >= 0 && str.indexOf(']') >= 0) {
            str = str.replace(/([\[])([\[\]\S]+?)[\s]+([^\[\]]+)?([\]])/g, function(match, p1, p2, p3, p4) {
                ...
                if (p2 == 'image') {
                            ...
                            if (!TRX_ADDONS_STORAGE['fetch_images'][atts['id']]) {
                                ...
                                jQuery.get(TRX_ADDONS_STORAGE['rest_url'] + 'wp/v2/media/' + atts['id'] + '/', function(response) {
                                    if (response && response['media_details'] && response['media_details']['sizes']) {
                                        ...
                                        jQuery('.trx_addons_image_' + atts['id']).removeClass('trx_addons_image_placeholder').attr('src', get_url_by_size(response, atts['thumb'] ? '-' + atts['thumb'] : '-tiny'));
                                    }
                                });
                            } else {
                    ...
                    return atts['url'] ? '<img src="' + atts['url'] + '"' + (atts['id'] ? ' id="trx_addons_image_' + atts['id'] + '"' : '') + (atts['alt'] ? ' alt="' + atts['alt'] + '"' : '') + (args['class'] || atts['class'] ? ' class="' + (args['class'] ? args['class'] + ' ' : '') + ((atts['class'] ? atts['class'] : '')) + '"' : '') + ' style="' + trx_addons_get_css_from_atts(atts, trx_addons_object_merge(allowed_css, image_css)) + (atts['css'] ? atts['css'] : '') + '"' + '>' : '';
                } else if (p2 == 'icon') {
                    atts = p3 ? trx_addons_parse_atts(p3) : {};
                    if (atts['name'] && atts['name'].slice(0, 5) != 'icon-') {
                        atts['name'] = 'icon-' + atts['name'];
                    }
                    return atts['name'] ? '<span class="' + atts['name'] + (args['class'] ? ' ' + args['class'] : '') + (atts['class'] ? ' ' + atts['class'] : '') + '"' + ' style="' + trx_addons_get_css_from_atts(atts, trx_addons_object_merge(allowed_css, icon_css)) + (atts['css'] ? atts['css'] : '') + '"' + '></span>' : '';
        ...
        for (var i = 0; i < buttons.length; i++) {
            if (settings[buttons[i]]) {
                if (typeof settings[buttons[i]] == 'object' && settings[buttons[i]]['value']) {
                    iconHTML = elementor.helpers.renderIcon(view, settings.selected_icon, {
                        'aria-hidden': true
                    }, 'span', 'object');
    ...
    window.trx_addons_elm_render_icon = function(view, icon_settings, defa) {
        ...
        if (args.value) {
            var output = elementor.helpers.renderIcon(view, args, {
                'aria-hidden': true
            }, 'i', 'value');
    ...
    window.trx_addons_esc_attr = function(str) {
        return !str ? str : str.replace(/"/g, '&quot;').replace(/'/g, '&#039;');
    };
    window.trx_addons_get_unique_id = function() {
        return Math.random().toString(16).slice(2, 9);
    };
    window.trx_addons_get_link_attributes = function(link_atts) {
        var rez = (link_atts.is_external ? ' target="_blank"' : '') + (link_atts.nofollow ? ' rel="nofollow"' : '');
            ...
            try {
                ...
                mediaRecorder.onstop = () => {
                    const mediaBlob = new Blob(chunks, {
                        type: ['video', 'both'].indexOf(options.type) >= 0 ? 'video/webm' : 'audio/wav'
                    });
    ...
    jQuery(document).ready(function() {
        ...
        function trx_addons_init_actions() {
            ...
            if (typeof TRX_ADDONS_STORAGE['animate_to_hash'] == 'undefined' && !$body.hasClass('single-product')) {
                ...
                setTimeout(function() {
                    ...
                    } else if (location.hash !== '' && location.hash != '#' && location.hash.indexOf('/') == -1 && location.hash.indexOf('&') == -1 && location.hash.indexOf(';') == -1 && location.hash.indexOf('%') == -1 && location.hash.indexOf('=') == -1) {
        ...
        function trx_addons_ready_actions(e, container) {
            ...
            if (TRX_ADDONS_STORAGE['animate_inner_links'] > 0 && !container.hasClass('animate_to_inited')) {
                container.addClass('animate_to_inited').on('click', 'a[href*="#"]', function(e) {
                    var link_obj = jQuery(this);
                    var link_parent = link_obj.parent();
                    if (link_obj.hasClass('trx_addons_panel_link') || link_obj.hasClass('trx_addons_popup_link') || link_obj.attr('href').indexOf('#elementor-action') >= 0 || link_parent.parent().hasClass('trx_addons_tabs_titles') || link_parent.parents('.wc-tabs').length > 0 || link_parent.hasClass('elementor-tab-title') || link_parent.parents('ul[class*="tabs"]').length > 0 || link_obj.hasClass('vc_pagination-trigger') || link_obj.hasClass('ui-tabs-anchor') || link_parent.hasClass('vc_tta-tab') || link_parent.hasClass('vc_tta-panel-title') || link_parent.hasClass('wpb_accordion_header')) {
                        return true;
                    }
            ...
            setTimeout(function() {
                container.find('.mejs-fullscreen-button:not(.resize_inited)').addClass('resize_inited').on('click', 'button', function() {
                    setTimeout(function() {
                        $window.trigger('resize');
                    }, 100);
                });
            ...
            if (TRX_ADDONS_STORAGE['popup_engine'] == 'pretty') {
                ...
                window.trx_addons_prepare_popup_content = function(container, autoplay) {
                    ...
                    if (wrapper.data('popup-content') === undefined) {
                        ...
                        if (wrapper.find('audio').length || wrapper.find('video').length || (iframe.length && ((iframe.data('src') && iframe.data('src').search(/(youtu|vimeo|daily|facebook|instagram.com)/i) > 0) || (iframe.attr('src') && iframe.attr('src').search(/(youtu|vimeo|daily|facebook|instagram.com)/i) > 0)))) {
            ...
            jQuery('.sc_layouts_menu_nav:not(.inited_cache),.trx-addons-nav-menu:not(.inited_cache),.trx-addons-mobile-menu:not(.inited_cache)').each(function() {
                ...
                if ($self.find('.current-menu-item').length == 0 || $body.hasClass('blog_template')) {
                    ...
                    var id = $self.attr('id');
                    if (id === undefined) {
                        id = ($self.hasClass('sc_layouts_menu_nav') ? 'sc_layouts_menu_nav_' : ($self.hasClass('trx-addons-nav-menu') ? 'trx-addons-nav-menu-' : 'trx-addons-mobile-menu-')) + trx_addons_get_unique_id();
                        $self.attr('id', id);
                    }
                        ...
                        if (row_top > wso) {
                            ...
                            if (force) {
                                ...
                                if (!use_sticky) {
                                    ...
                                    row.css({
                                        top: fixed_rows_height + 'px !important'
                                    });
                                }
                            }
                            if (row.hasClass('sc_stack_section_effect_fade')) {
        ...
        var trx_addons_set_active_menu_items = trx_addons_throttle(function() {
            ...
            let $lastValidSectionMenuLink = null;
            $sections_with_id.each(function() {
                ...
                if (sectionTop <= screenMiddle && $menuLink.length) {
                    if ($lastValidSection && $lastValidSection.length && $lastValidSection.data('menu-active')) {
                        $lastValidSection.data('menu-active', false);
                        clearActiveClasses($lastValidSection.attr('id'));
                    }
                    $lastValidSection = $section;
                    $lastValidSectionMenuLink = $menuLink;
                } else if ($section.data('menu-active')) {
                    $section.data('menu-active', false);
                    clearActiveClasses($section.attr('id'));
                }
            });
            if ($lastValidSection && $lastValidSection.length && !$lastValidSection.data('menu-active')) {
            ...
            function setActiveMenuItem($menuLink, $section) {
                $menuLink.each(function() {
                    clearActiveClasses();
                    jQuery(this).closest('li').addClass('current-menu-item trx-addons-active-item').parents('li').each(function(index) {
        ...
        function trx_addons_resize_video(e, cont) {
            if ($video_tags.length > 0) {
                $video_tags.each(function() {
                    ...
                    if ((!TRX_ADDONS_STORAGE['resize_tag_video'] && $self.parents('.mejs-mediaelement').length === 0) || $self.hasClass('trx_addons_noresize') || classes.indexOf('_resize') > 0 || classes.indexOf('_noresize') > 0 || $self.hasClass('sr7-media') || $self.parent().is('rs-bgvideo') || $self.parents('rs-slide,div:hidden,section:hidden,article:hidden').length > 0) {
                        return;
                    }
        ...
        if (TRX_ADDONS_STORAGE['login_via_ajax'] && !error) {
            ...
            jQuery.post(TRX_ADDONS_STORAGE['ajax_url'], {
                ...
                if (rez.error === '') {
                    ...
                    setTimeout(function() {
                        var new_url = rez.redirect_to !== '' ? (rez.redirect_to.indexOf('action=confirm_email') != -1 ? rez.redirect_to.substr(0, rez.redirect_to.indexOf('?')) : rez.redirect_to) : (location.href.indexOf('action=confirm_email') != -1 ? TRX_ADDONS_STORAGE['site_url'] : location.href);
                        new_url = trx_addons_add_to_url(new_url, {
                            rnd: trx_addons_get_unique_id()
                        });
...
(function() {
            ...
            if (is_edit_mode) {
                ...
                jQuery('.trx_addons_layout_editor_selector_list_item:not(.inited)').addClass('inited').on('click', function(e) {
                    ...
                    if (layout_id && layout_type && post_id && elementor) {
                        ...
                        elementor.getPanelView().setPage('page_settings').activateTab('advanced').activateSection('section_theme_options_' + sec_number)._renderChildren();
            ...
            for (var i = 0; i < settings.tabs.length; i++) {
                ...
                if (tab) {
                    ...
                    if ($tab.length) {
                        ...
                        if (title_tag != 'span') {
                            var $newText = jQuery('<' + title_tag + '></' + title_tag + '>', {
                                html: $text.html()
                            });
                        ...
                        if (tab.tab_subtitle) {
                            $tab.toggleClass('e-n-tab-title-with-subtitle', true).toggleClass('e-n-tab-title-icon-placement-inside', icon_placement == 'inside').toggleClass('e-n-tab-title-icon-placement-outside', icon_placement == 'outside');
                            ...
                            $tab.removeClass('e-n-tab-title-with-subtitle e-n-tab-title-icon-placement-inside e-n-tab-title-icon-placement-outside').find('.e-n-tab-subtitle').remove().end().find('.e-n-tab-title-text-inner').contents().unwrap();
                        }
                    }
                }
            }
    ...
    window.trx_addons_elementor_prepare_animate_items = function(force, $el) {
        ...
        var $check_items = force && $el && $el.length ? $el : jQuery('[class*="animation_type_"]:not(.animation_type_block)' + (!force ? ':not(.animated-separate)' : ''));
        $check_items.each(function() {
            ...
            if (items.length && items.eq(0).parent().hasClass('swiper-wrapper') && !items.eq(0).parent().parent().hasClass('swiper-initialized') && !items.eq(0).parent().parent().hasClass('swiper-container-initialized')) {
            ...
            if (!cid) {
                ...
                setTimeout(function() {
                    ...
                    if (items.length) {
                        items.removeClass('animated animated-slow animated-fast ' + trx_addons_elementor_animate_items_animation(item_params)).addClass('elementor-invisible trx_addons_invisible');
                    }
                ...
                if (['line', 'word', 'char'].indexOf(animation_type) >= 0 && allowed.indexOf(animation_type) >= 0) {
                    ...
                    if (item.data('animation-prepared') === undefined) {
                        ...
                        if (animation_type == 'word' || animation_type == 'line') {
                            html = trx_addons_wrap_words(html, '<span class="sc_item_animated_block elementor-invisible trx_addons_invisible animated-item' + (item_speed ? ' ' + item_speed : '') + '">', '</span>');
                        } else {
                            html = trx_addons_wrap_chars(html, '<span class="sc_item_animated_block elementor-invisible trx_addons_invisible animated-item' + (item_speed ? ' ' + item_speed : '') + '">', '</span>', '<span class="sc_item_word">', '</span>', );
                        }
    ...
    function trx_addons_elementor_animate_item(item, idx, total, force, need_animation) {
        ...
        if (!need_animation) {
            ...
            setTimeout(function() {
                item.removeClass('wait-for-animation').addClass('animated' + (item_duration ? ' animated-' + item_duration : '')).addClass(item_animation).removeClass('elementor-invisible trx_addons_invisible');
            }, item_delay);
        }
    }
    ...
    function trx_addons_elementor_animate_items_delay(params, sc, item, idx, total) {
        var animation_type = trx_addons_get_class_by_prefix(sc.attr('class'), 'animation_type_').replace('animation_type_', '');
        var delay = params && params._animation_delay !== '' && params._animation_delay !== undefined ? params._animation_delay : (params && params.animation_delay !== '' && params.animation_delay !== undefined ? params.animation_delay : 0);
        var stagger = params && params._animation_stagger !== '' && params._animation_stagger !== undefined ? params._animation_stagger : sc.data('animation-stagger');
    ...
    function trx_addons_elementor_add_bg_text(cont) {
        if (cont.hasClass('elementor-section') || cont.hasClass('e-con')) {
            ...
            jQuery((is_edit_mode ? '.elementor-section.elementor-element-edit-mode,.e-con.elementor-element-edit-mode' : '.trx_addons_has_bg_text:not(.trx_addons_has_bg_text_inited)')).each(function() {
                var $self = jQuery(this);
                if (is_edit_mode) {
    ...
    function trx_addons_elementor_add_bg_text_in_row(row) {
        ...
        if (data['bg_text']) {
            ...
            data['bg_text'] = trx_addons_prepare_macros(data['bg_text'], {
                'class': 'trx_addons_bg_text_item'
            });
            ...
            if (chars.indexOf('[divider]') > 0) {
                if (delimiter_icon || delimiter_image) {
                    chars = chars.replaceAll('[divider]', (data['bg_text_effect'] == 'none' ? '</span>' : '') + '<span class="trx_addons_bg_text_item trx_addons_bg_text_char' + ' trx_addons_bg_text_delimiter' + (!delimiter_image && delimiter_icon ? ' trx_addons_bg_text_delimiter_icon ' + delimiter_icon : ' trx_addons_bg_text_delimiter_image') + (data['bg_text_delimiter_rotation'] > 0 ? ' trx_addons_bg_text_delimiter_rotation' : '') + '">' + (delimiter_image ? (delimiter_svg ? delimiter_svg : '<img src="' + delimiter_image + '" />') : '') + '</span>' + (data['bg_text_effect'] == 'none' ? '<span class="trx_addons_bg_text_item trx_addons_bg_text_char">' : ''));
                } else {
        ...
        if (elements.eq(0).hasClass('trx_addons_showed_on_scroll')) {
            ...
            elements.each(function(idx) {
                    ...
                    if (idx == elements.length - 1) {
                        ...
                        if (tween_obj) {
                            ...
                            elements.each(function(idx2) {
                                ...
                                elements.eq(idx2).data('tweenmax-object', TweenMax.to(elements.eq(idx2), 0, {
                                    overwrite: true,
                            ...
                            if (delimiters_loop > 0) {
                                delimiters.each(function(idx2) {
                                    ...
                                    delimiters.eq(idx2).data('tweenmax-delimiter', TweenMax.to(delimiters.eq(idx2), 0, {
                                        overwrite: true,
                                        rotation: 0,
                                        ease: Power0.easeNone
                                    }));
                ...
                if (delimiters_loop > 0 && idx == elements.length - 1) {
                    delimiters.each(function(idx2) {
                        kill_tween(delimiters.eq(idx2), 'tweenmax-delimiter');
                        delimiters.eq(idx2).data('tweenmax-delimiter', TweenMax.to(delimiters.eq(idx2), mw * time_per_pixel, {
                            overwrite: true,
            ...
            if (pause_on_hover) {
                marquee_wrap.on('mouseenter', function() {
                    elements.each(function(idx2) {
            ...
            var wheel_time = 0,
                ...
                wheel_handler = function(e) {
                    ...
                    elements.eq(0).data('tweenmax-accelerate', TweenMax.to(wheel_accelerate, 2.0, {
                        ...
                        ease: Power4.easeOut,
                        onUpdate: function() {
                            var coeff = wheel_accelerate.value == 1 ? 1 : (wheel_accelerate.value <= accelerate_coef ? wheel_accelerate.value : accelerate_coef * 2 - wheel_accelerate.value);
                            elements.each(function(idx2) {
                                var tween_obj = elements.eq(idx2).data('tweenmax-object');
                                if (tween_obj) {
...
(function($) {
    ...
    var widgetUpdate = function(slider, selector, type) {
        ...
        var $triggers = ['trx-addons-action-tabs-switched', 'trx-addons-action-toggle-switched', 'trx-addons-action-accordion-switched', 'trx-addons-action-popup-opened', ];
...
jQuery(document).on('action.ready_trx_addons', function() {
        ...
        if (rows.length > 0) {
            rows.each(function(idx) {
                ...
                if (!row_next.hasClass('sc_layouts_row_fixed_placeholder')) {
                    if (row_next.hasClass('vc_row-full-width')) {
                        row_next.addClass('sc_layouts_row_fixed_placeholder').css('background-color', row.css('background-color')).append('<div class="sc_layouts_row_fixed_marker_off"></div>');
                    } else {
                        ...
                        observer = new IntersectionObserver(function(entries) {
                            ...
                            entries.forEach(function(entry) {
                                ...
                                if (row.hasClass('sc_layouts_row_fixed_on') && entry.isIntersecting) {
                                    if (time - last_fixed_time > animation_off_timeout && entry.boundingClientRect && entry.rootBounds && entry.boundingClientRect.top >= entry.rootBounds.top) {
                                    ...
                                    if (time - last_fixed_time > animation_off_timeout && entry.boundingClientRect && entry.rootBounds && entry.boundingClientRect.bottom < entry.rootBounds.top) {
                                        trx_addons_cpt_layouts_fix_rows({
                                            rows: rows,
                                            rows_always: rows_always,
                                            force_row: row,
    ...
    function trx_addons_cpt_layouts_fix_rows(args) {
        ...
        rows.each(function(idx) {
            ...
            var offset = parseInt(row.hasClass('sc_layouts_row_fixed_on') || row.hasClass('sc_layouts_row_fixed_on_pre') ? placeholder.offset().top : (row.hasClass('sc_layouts_row_hide_unfixed') ? row_unfixed_placeholder.offset().top : row.offset().top), 10);
            if (isNaN(offset)) {
                offset = 0;
            }
            if (scroll_intent > 1 && ((force_state == 'off' && row.is(force_row)) || (!force_state && (scroll_offset + rows_offset <= offset + delay || (scroll_offset == 0 && last_scroll_offset > 0) || h - ph < -2) && (!row.hasClass('sc_layouts_row_fixed_from_start') || scroll_offset > 0)))) {
                ...
                if (!row.hasClass('sc_layouts_row_fixed_on')) {
                    if (scroll_intent <= 0 && ((force_state == 'on' && row.is(force_row)) || scroll_offset + rows_offset > offset + delay)) {
                        ...
                        if (!row.hasClass(fixed_class)) {
                            ...
                            row.addClass('sc_layouts_row_fixed_on' + (!row.hasClass('sc_layouts_row_fixed_from_start') && scroll_offset == 0 && scroll_intent == 0 ? '_pre' : '')).css({
                                'top': rows_offset + 'px'
                            });
                            h = row.outerHeight();
                            if (ph != h && !row.hasClass('sc_layouts_row_hide_unfixed')) {
    ...
    function trx_addons_init_sliders(e, container) {
        ...
        if (window.Swiper) {
            container.find('.sc_slider_controller:not(.inited)').each(function() {
                ...
                var controller_icon_prev = controller_icon_custom ? controller.data('controls-icon-prev') : (controller_icon ? controller_icon : 'slider_arrow_default');
                var controller_icon_next = controller_icon_custom ? controller.data('controls-icon-next') : (controller_icon_prev ? controller_icon_prev.replace('left', 'right').replace('up', 'down') : '');
                var controller_html = '';
            ...
            container.find('.sc_slider_controls:not(.inited)').each(function() {
                ...
                if (pagination_style == 'thumbs') {
                    ...
                    slides.each(function(idx) {
                        ...
                        html += trx_addons_apply_filters('trx_addons_filter_slider_controls_html_title', '<' + pagination_title_html_tag + ' class="slider_pagination_title swiper-pagination-title">' + title + '</' + pagination_title_html_tag + '>', slide, controls, );
                    });
                    controls.find('.slider_pagination_wrap').html(trx_addons_apply_filters('trx_addons_filter_slider_controls_html_titles', html, controls));
                }
                        ...
                        if (TRX_ADDONS_STORAGE['swipers'][slider_id]) {
                            ...
                            if (total > 0) {
                                if (pagination_style == 'progressbar') {
                                    ...
                                    controls.find('.slider_pagination_title').removeClass('slider_pagination_title_active swiper-pagination-title-active').eq(current - 1).addClass('slider_pagination_title_active swiper-pagination-title-active');
                                }
                            }
                        }
                    });
    ...
    function trx_addons_resize_sliders(e, container) {
        ...
        container.find('.slider_container.inited').each(function() {
            ...
            if (last_width === 0 || last_width != slider_width) {
                ...
                if (slider.hasClass('slider_swiper') && typeof TRX_ADDONS_STORAGE['swipers'] != 'undefined' && typeof TRX_ADDONS_STORAGE['swipers'][id] == 'object' && typeof TRX_ADDONS_STORAGE['swipers'][id].params == 'object') {
...
(function() {
    ...
    $document.on('action.before_ready_trx_addons', function() {
        ...
        jQuery('.sc_layouts_menu:not(.inited_kbd)' + ',.menu_mobile_nav_area:not(.inited_kbd)' + ',.trx-addons-nav-menu-container:not(.inited_kbd)' + ',.trx-addons-mobile-menu-container:not(.inited_kbd)').addClass('inited_kbd').on('keydown', 'li > a', function(e) {
            var handled = false,
                link = jQuery(this),
                li = link.parent(),
                ul = li.parent(),
    ...
    function trx_addons_menu_collapse() {
        ...
        jQuery('.sc_layouts_menu:not(.sc_layouts_menu_no_collapse):not(.sc_layouts_menu_dir_vertical),.elementor-widget-trx_elm_nav_menu.trx-addons-nav-hor .trx-addons-nav-menu-container-collapse').each(function() {
            ...
            ul.find('> li').each(function(idx) {
                ...
                var cur_item_width = !cur_item.hasClass(li_collapse_class) || cur_item.css('display') != 'none' ? cur_item.outerWidth() + Math.ceil(parseFloat(cur_item.css('marginLeft'))) + Math.ceil(parseFloat(cur_item.css('marginRight'))) : 0;
            ...
            if (move) {
                ...
                items.each(function(idx) {
                    ...
                    if (w_all > w_max && cnt_visible >= visible) {
                        cur_item.attr('class', cur_item.attr('class').replace('trx_addons_stretch_', 'trx_addons_nostretch_')).find('> ul').attr('style', '');
                        if (li_collapse_new) {
                            cur_item.addClass('trx-addons-submenu-item').find('> a').addClass('trx-addons-submenu-link').find('> .trx-addons-menu-link-text').addClass('trx-addons-submenu-link-text').find('> .trx-addons-dropdown-icon').html(nav.data('collapse-submenu-icon'));
                        }
                        var moved = false;
                ...
                items.each(function() {
                    ...
                    if (w_all < w_max) {
                        if (li_collapse_new) {
                            $item.removeClass('trx-addons-submenu-item').find('> a').removeClass('trx-addons-submenu-link').find('> .trx-addons-menu-link-text').removeClass('trx-addons-submenu-link-text').find('> .trx-addons-dropdown-icon').html(nav.data('collapse-menu-icon'));
                        }
                        $item.attr('class', $item.attr('class').replace('trx_addons_nostretch_', 'trx_addons_stretch_')).insertBefore(li_collapse);
                        cnt++;
                    } else {
                        move = false;
                    }
    ...
    window.trx_addons_stretch_submenu = function(menu_item) {
        ...
        if (TRX_ADDONS_STORAGE['menu_stretch'] == 1 && !menu_item.hasClass('trx_addons_no_stretch') && parent_class.indexOf('trx_addons_no_stretch') == -1 && !menu_item.parents('.sc_layouts_menu').hasClass('sc_layouts_menu_dir_vertical') && trx_addons_apply_filters('trx_addons_filter_stretch_menu', menu_item.hasClass('sc_layouts_submenu') || parent_class.indexOf('columns-') != -1 || parent_class.indexOf('trx_addons_stretch_') != -1, menu_item)) {
    ...
    window.trx_addons_shift_submenu = function(menu_item) {
        ...
        if (menu_item.parents(TRX_ADDONS_STORAGE['header_wrap_class']).length > 0) {
            ...
            if (!wide) {
                ...
                var ul_offset = menu_item.parents("ul").length > 1 ? par_offset + ul_pos : (submenu_centered ? par_offset + par_width / 2 - ul_width / 2 : menu_item.parent().offset().left);
                if (submenu_left) {
                    if (ul_offset < 0) {
                ...
                if ((search_wrap.hasClass('search_style_expand') || search_wrap.hasClass('search_style_fullscreen')) && !search_wrap.hasClass('search_opened')) {
                    ...
                    if (search_wrap.hasClass('search_style_fullscreen')) {
                        ...
                        if (animation) {
                            ...
                            search_wrap.find('.search_form_wrap').addClass(animation + ' animated' + (animation_duration ? ' animated-' + animation_duration : ''));
                            search_wrap.find('.search_form_overlay').addClass('animated' + (animation_duration ? '-' + animation_duration : ''));
                        } else {
                            search_wrap.find('.search_form_overlay').toggleClass('trx_addons_no_transition', true);
                        }
    ...
    function trx_addons_search_close(search_wrap) {
        ...
        if (search_wrap.hasClass('search_style_fullscreen')) {
            ...
            var on_end_animation = function() {
                ...
                search_button.addClass('trx_addons_no_transition');
                search_wrap.removeClass('search_opened').removeAttr('style').show();
                search_wrap.find('.search_form_overlay').removeClass('search_form_overlay_hide');
                search_button.removeClass('trx_addons_no_transition');
            };
            ...
            if (animation_exit) {
                ...
                trx_addons_on_end_animation(search_form_wrap.get(0), function() {
                    search_form_wrap.removeClass(animation_exit ? animation_exit + ' animated' + (animation_duration ? ' animated-' + animation_duration : '') : '');
                    on_end_animation();
